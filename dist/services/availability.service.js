...
async function getAvailableSlots(date, serviceId, addonIds = [], staffId) {
    ...
    const [service] = await (0, db_1.query)(`SELECT duration_min FROM services WHERE id = $1`, [serviceId]);
    if (!service)
        throw new Error("Service not found");
    // 2. Get addon duration
    let addonDuration = 0;
    if (addonIds.length) {
        const addons = await (0, db_1.query)(`SELECT extra_duration_min FROM addons WHERE id = ANY($1)`, [addonIds]);
    ...
    const staffList = staffId
        ...
        : await (0, db_1.query)(`SELECT DISTINCT s.id
         ...
         WHERE ss.service_id = $1 AND s.active = true`, [serviceId]);
    const slots = [];
    for (const staff of staffList) {
        // 4. Get working hours
        const availability = await (0, db_1.query)(`SELECT start_time, end_time
       ...
       AND day_of_week = EXTRACT(DOW FROM DATE $2)`, [staff.id, date]);
        ...
        const busy = await (0, db_1.query)(`SELECT start_at, end_at
       FROM appointments
       WHERE staff_id = $1
       ...
       AND DATE(start_at) = DATE $2`, [staff.id, date]);
        while (cursor.getTime() + totalDuration * 60000 <= endOfDay.getTime()) {
            ...
            const overlaps = busy.some(b => cursor < new Date(b.end_at) && slotEnd > new Date(b.start_at));
            if (!overlaps) {
                slots.push({
                    staff_id: staff.id,
